...
function TimeBlock({ item }: { item: ITimeBlockResponse }) {
	...
	return (
		...
		>
			<div
				...
				style={{ backgroundColor: item.color || 'lightgray', height: `${item.duration}px` }}
			>
				<div className='flex items-center'>
					...
					<div>
						{item.name} <i className='tex-xs opacity-50'>({item.duration} min.)</i>
					</div>
				</div>
